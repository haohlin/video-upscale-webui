...
load_runtime_config() {
  ...
  : "${VIDEO_UPSCALE_TAILSCALE_USER_LOGIN:?runtime config missing VIDEO_UPSCALE_TAILSCALE_USER_LOGIN}"
  ...
  [[ "$VIDEO_UPSCALE_TAILSCALE_USER_LOGIN" != *$'\n'* ]] \
    || die "VIDEO_UPSCALE_TAILSCALE_USER_LOGIN must not contain a newline"
  ...
  for numeric_value in VIDEO_UPSCALE_FFPROBE_TIMEOUT_SECONDS VIDEO_UPSCALE_UPLOAD_IDLE_TIMEOUT_SECONDS \
    ...
    VIDEO_UPSCALE_MAX_SOURCE_FRAMES VIDEO_UPSCALE_MAX_PENDING_JOBS \
  ...
  export VIDEO_UPSCALE_TAILSCALE_USER_LOGIN
  ...
  export VIDEO_UPSCALE_MAX_PENDING_JOBS
  ...
  for optional_name in VIDEO_UPSCALE_BACKENDS_JSON VIDEO_UPSCALE_BACKEND_ID \
    VIDEO_UPSCALE_BACKEND_DISPLAY_NAME VIDEO_UPSCALE_PLATFORM_NAME \
    VIDEO_UPSCALE_ACCELERATOR_NAME VIDEO_UPSCALE_DEVICE_BACKEND_CLASS \
    VIDEO_UPSCALE_ALLOWED_WEB_ORIGIN; do
    [[ -z "${(P)optional_name:-}" ]] || export "$optional_name"
  done
